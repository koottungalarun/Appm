...
void Mesh::writeToFile()
{
	...
	if (nVertices > 0) {
		...
		h5writer.writeDataInt(vertexType, "/vertexType");
	}
	...
	if (nEdges > 0) {
		...
		h5writer.writeDataDouble(edgeLength, "/edgeLength");
		...
		h5writer.writeDataInt(edge2Vertex, "/edge2vertex");

		Eigen::VectorXi edgeIdx(nEdges);
		for (int i = 0; i < nEdges; i++) {
			edgeIdx(i) = getEdge(i)->getIndex();
		}
		h5writer.writeDataInt(edgeIdx, "/edgeIdx");
		...
		h5writer.writeDataInt(edgeType, "/edgeType");
	}
	...
	if (nFaces > 0) {
		...
		h5writer.writeDataDouble(fc, "/faceCenter");
		h5writer.writeDataInt(faceIdx, "/faceIndex");
		h5writer.writeDataInt(faceBoundary, "/isFaceBoundary");
		h5writer.writeDataDouble(fn, "/faceNormal");
		assert((faceArea.array() > 0).all());
		h5writer.writeDataDouble(faceArea, "/faceArea");
		h5writer.writeDataInt(faceType, "/faceFluidType");
	...
	if (nCells > 0) {
		...
		h5writer.writeDataDouble(cellCenters, "/cellCenter");
		h5writer.writeDataInt(cellType, "/cellFluidType");

		Eigen::VectorXi cellIdx(nCells);
		for (int i = 0; i < nCells; i++) {
			cellIdx(i) = getCell(i)->getIndex();
		}
		h5writer.writeDataInt(cellIdx, "/cellIndex");
		...
		h5writer.writeDataDouble(cellVolume, "/cellVolume");
	}
}
...
XdmfGrid Mesh::getXdmfVertexGrid() const
{
	...
	{
		std::stringstream ss;
		ss << getMeshDataFilename() << ":/vertexIdx";
	...
	{
		std::stringstream ss;
		ss << getMeshDataFilename() << ":/vertexPos";
	...
	{
		std::stringstream ss;
		ss << getMeshDataFilename() << ":/vertexType";
...
XdmfGrid Mesh::getXdmfEdgeGrid() const
{
	...
	{
		std::stringstream ss;
		ss  << getMeshDataFilename() << ":/edge2vertex";
	...
	{
		std::stringstream ss;
		ss << getMeshDataFilename() << ":/edgeIdx";
	...
	{
		std::stringstream ss;
		ss << getMeshDataFilename() << ":/edgeType";
...
{
	H5Reader h5reader(getMeshDataFilename());
	...
	{
		std::stringstream ss;
		ss << getMeshDataFilename() << ":/face2vertex";
	...
	// Attribute: face index
	{
		...
		std::stringstream ss;
		ss << getMeshDataFilename() << ":/faceIndex";
	...
	std::stringstream ss;
	ss << getMeshDataFilename() << ":/faceArea";
	...
	{
		std::stringstream ss;
		ss << getMeshDataFilename() << ":/faceNormal";
		std::string faceNormalStringAttribute = ss.str();
...
XdmfGrid Mesh::getXdmfVolumeGrid() const
{
	...
	{
		std::stringstream ss;
		ss << getMeshDataFilename() << ":/cell2vertex";
	...
	// Attribute: cell index
	{
		...
		std::stringstream ss;
		ss << getMeshDataFilename() << ":/cellIndex";
	...
	{
		std::stringstream ss;
		ss << getMeshDataFilename() << ":/cellVolume";
...
void Mesh::writeXdmfVolumeMesh() const
{
...

	{
		...
		}
		{
			// Geometry and Geometry DataItem
			std::stringstream ss;
			ss << getMeshDataFilename() << ":/cellCenter";
			XdmfGeometry geometry;
			geometry.addChild(
				XdmfDataItem(
					XdmfDataItem::Tags(
