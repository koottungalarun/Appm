...
void DualMesh::init_dualMesh(const PrimalMesh & primal, const double terminalRadius)
{
	...
	primalFaceToDualVertex = Eigen::SparseVector<int>(nPrimalFaces);
	primalEdgeToDualVertex = Eigen::SparseVector<int>(nPrimalEdges);
	primalVertexToDualVertex = Eigen::SparseVector<int>(nPrimalVertices);

	init_dualVertices(primal);
	init_dualEdges(primal);
	assert(validateEdges());

	init_dualFaces(primal);
	assert(validateFaces());

	init_dualCells(primal);
	assert(validateCells());
...
Eigen::VectorXi DualMesh::associateDualFacesWithPrimalEdges(const PrimalMesh & primal)
{
	...
	if (temp.minCoeff() < 0.9) {
		...
		std::cout << "*****************************" << std::endl;
		std::cout << "* Warning: refine the mesh! *" << std::endl;
		std::cout << "*****************************" << std::endl;
	}
...
void DualMesh::init_dualVertices(const PrimalMesh & primal)
{
	std::cout << "Init dual vertices" << std::endl;
	...
	for (int i = 0; i < nPrimalEdges; i++) {
		...
		if (edge->isBoundary()) {
			...
			if (fn0.cross(fn1).norm() > 4 * std::numeric_limits<double>::epsilon()) {
				...
				newPos.setZero();
				if (fn0(2) == 0 && fn1(2) == 0) {
					newPos = edge->getHalfwayPosition();
					Eigen::Vector3d fc0 = boundaryFaces[0]->getCenter();
					Eigen::Vector3d fc1 = boundaryFaces[1]->getCenter();
					assert((fc1 - fc0)(2) == 0);
					newPos(2) = fc0(2);
					V = addVertex(newPos);
				}
					...
					if (fn0(2) == 0) {
						temp.segment(0, 2) = boundaryFaces[0]->getCenter().segment(0, 2); // (x,y)-component
						temp(2) = boundaryFaces[1]->getCenter()(2); // z-component
					}
					else {
						temp.segment(0, 2) = boundaryFaces[1]->getCenter().segment(0, 2); // (x,y)-component
						temp(2) = boundaryFaces[0]->getCenter()(2); // z-component
					}
					V = addVertex(temp);
				}
				...
				//bool showOutput = V->getIndex() == 13776;
				//if (showOutput) {
				//	std::cout << std::scientific;
				//	std::cout << "Vertex idx: " << V->getIndex() << std::endl;
				//	std::cout << "Vertex pos: " << V->getPosition().transpose() << std::endl;
				//	std::cout << "fn0:    " << fn0.transpose() << std::endl;
				//	std::cout << "fn1:    " << fn1.transpose() << std::endl;
				//	std::cout << "temp:   " << temp.transpose() << std::endl;
				//	std::cout << "newPos: " << newPos.transpose() << std::endl;
				//	std::cout << "fc0:    " << boundaryFaces[0]->getCenter().transpose() << std::endl;
				//	std::cout << "fc1:    " << boundaryFaces[1]->getCenter().transpose() << std::endl;
				//}
			}
		}
	}
...
void DualMesh::init_dualEdges(const PrimalMesh & primal)
{
	std::cout << "Init dual edges" << std::endl;
...
void DualMesh::init_dualFaces(const PrimalMesh & primal)
{
	std::cout << "Init dual faces" << std::endl;
...
void DualMesh::init_dualCells(const PrimalMesh & primal)
{
	std::cout << "Init dual cells" << std::endl;

	const int nPrimalVertices = primal.getNumberOfVertices();

	for (int i = 0; i < nPrimalVertices; i++) {

		//if (i > 7570) { std::cout << i << std::endl; }
		bool showOutput = false; // i == 6517;
		...
		if (showOutput) {
			std::cout << "Create dual cell at primal vertex " << primalVertex->getIndex() << std::endl;
			std::cout << "List of primal edges:" << std::endl;
			std::cout << std::scientific;
			for (auto edge : primalVertexEdges) {
				std::cout << "idx: " << edge->getIndex() << ", ";
				std::cout << "pos: " << edge->getHalfwayPosition().transpose() << std::endl;
			}
		}
		...
		if (hasPrimalBoundaryEdges) {
			...
			for (auto primalEdge : primalVertexEdges) {
				...
				if (primalEdge->isBoundary()) {
					...
					if (primalBoundaryFaces.size() == 2) {
						if (showOutput) {
							std::cout << "Primal boundary faces: " << std::endl;
							for (auto face : primalBoundaryFaces) {
								std::cout << "idx: " << face->getIndex() << ", ";
								std::cout << "pos: " << face->getCenter().transpose() << std::endl;
							}
						}
			...
			if (showOutput) {
				std::cout << "List of dual boundary edges: " << std::endl;
			...
			if (showOutput) {
				std::cout << "List of dual faces: " << std::endl;
				for (auto face : dualFaces) {
					std::cout << "idx: " << face->getIndex() << ", ";
					std::cout << "pos: " << face->getCenter().transpose();
					std::cout << std::endl;
					std::vector<Vertex*> facevertices = face->getVertexList();
					std::cout << "vertices: " << std::endl;
					for (auto vertex : facevertices) {
						std::cout << vertex->getIndex() << ": " << vertex->getPosition().transpose() << std::endl;
					}
				}
			}
			...
			if (showOutput) {
				std::cout << "Create dual boundary face from this list of edges" << std::endl;
			}
			...
			while (dualBoundaryEdges.size() > 0) {
				...
				if (showOutput) {
					std::cout << "Selected edges loop:" << std::endl;
					for (auto edge : dualBoundaryEdges) {
						std::cout << "idx: " << edge->getIndex() << ", ";
						std::cout << "pos: " << edge->getHalfwayPosition().transpose() << std::endl;
					}
				...
				if (showOutput) {
					std::cout << "Face is created of edges:" << std::endl;
					std::cout << "idx: " << face->getIndex() << ", ";
					std::cout << "pos: " << face->getCenter().transpose() << std::endl;
			...
			if (showOutput) {
				std::cout << "Create dual cell from faces" << std::endl;
				for (auto face : dualFaces) {
					std::cout << "idx: " << face->getIndex() << ", ";
					std::cout << "pos: " << face->getCenter().transpose();
					std::cout << std::endl;
